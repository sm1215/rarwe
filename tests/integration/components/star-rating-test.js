...
module('Integration | Component | star-rating', function(hooks) {
  ...
  test('Renders the full and empty stars correctly', async function(assert) {
    ...
    assert.dom('.fa-star-o').exists({ count: 6 }, 'The right amount of full stars is rendered after changing rating');

    this.set('rating', 2);
    assert.dom('.fa-star').exists({ count: 2 }, 'The right amount of stars is rendered after changing rating');
